...
class contrail::config (
  $keystone_host,			## Keystone host
  $keystone_admin_token,		## Keystone Admin token
  $keystone_auth_password,		## neutron user password
  $neutron_port = 9697,			## neutron port
  $rabbitmq_host = $ipaddress,		## Rabbit mq server host
  $keystone_admin_port = 35357,		## Keystone Admin port
  $keystone_protocol = 'http',		## Keystone protocol
  $service_tenant = 'services',		## Keystone service tenant name
  $keystone_version = 'v2.0',		## Keystone version
# $neutron_ip = $ipaddress,
  $neutron_protocol = 'http',		## Neutron protocol
# $neutron_port = 9697,
  $contrail_control_ip_list = [ $ipaddress ],  	## Contrail control node ip list
  $contrail_api_port_base = '910',		## the port to start listening - one port per api workers
  $contrail_api_workers = 1, 			## Number of contrail api workers
  $contrail_supervisorctl_lines = 'supervisorctl -s http://localhost:9004 ${1} `basename ${0}:0`',  ## Need more information to fix this
){
...
define config-template-scripts {
  ...
  if ! defined(Exec["api-venv"]) {
    exec { "api-venv" :
      ...
      unless    => ["[ ! -f /opt/contrail/api-venv/archive/[a-zA-Z0-9][a-zA-Z0-9]* ]",
  ...
  ## Setup neutron.conf
  if ! defined(File["/etc/neutron/neutron.conf"]) {
    file { '/etc/neutron/neutron.conf':
       ensure  => file,
       owner   => neutron,
       group   => neutron,
       mode    => 640,
       content => template("$module_name/neutron.conf.erb")
    }

  ### /etc/ifmap-server doesnt exists  with contrail version 1.05 - it has /etc/irond/,
  ## As a workaround creating a softlink 
  exec {'makeLink__etc_ifmap-server':
    command  => 'ln -s /etc/irond /etc/ifmap-server',
    path     => '/bin:/usr/bin:/sbin:/usr/sbin',
    onlyif   => ["[ -d /etc/irond ]", "[ ! -e /etc/ifmap-server ]" ],
  }
  ...
  # Ensure ifmap.properties file is present with right content.
    ...
    file { "/etc/ifmap-server/log4j.properties" : 
        ...
        require => Package["contrail-openstack-config"],
        source => "puppet:///modules/$module_name/log4j.properties",
    }
    ...
    file { "/etc/ifmap-server/authorization.properties" : 
        ...
        require => Package["contrail-openstack-config"],
        source => "puppet:///modules/$module_name/authorization.properties",
    }
